...
export default function Details1({ product }) {
  ...
  useEffect(() => {
    if (product?.colors?.length > 0) {
      setActiveColor(product.colors[0].name.toLowerCase());
    }
  }, [product]);
  ...
  const imgSrcUrl = getImageUrl(product?.imgSrc);
  const imgHoverUrl = getImageUrl(product?.imgHover);
  ...
  if (imgSrcUrl) {
    slideItems.push({
      id: 1,
      color: product.colors?.[0]?.name?.toLowerCase() || "gray",
      src: imgSrcUrl,
      alt: product.title,
      width: 600,
      height: 800,
  ...
  if (product.colorImages?.length > 0) {
    product.colorImages.forEach((ci) => {
      const colorObj = product.colors?.find(
        (c) => c._id === ci.color || c.id === ci.color
      );
      const url = getImageUrl(ci.imageUrl);
      if (url) {
        slideItems.push({
          id: slideItems.length + 1,
          color: colorObj?.name?.toLowerCase() || "gray",
          src: url,
          alt: product.title,
          width: 600,
          height: 800,
                ...
                <div className="tf-product-info-list other-image-zoom">
                  <div className="tf-product-info-heading">
                    <div className="tf-product-info-name">
                      <div className="text text-btn-uppercase">luxury</div>
                    ...
                    <div className="tf-product-info-desc">
                      ...
                      <p>
                        {product.description || "Serenabelle Wingback Bed – Heritage-Inspired Bedroom Classic"}
                      </p>
