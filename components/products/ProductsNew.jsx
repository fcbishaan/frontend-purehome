        ...
        const fetchData = async () => {
            try {
                ...
                if (productRes.data.success) {
                    ...
                    console.log("Fetched and mapped products:", mappedProducts);

                    // Dynamically set price range based on products
                    if (mappedProducts.length > 0) {
                        const prices = mappedProducts.map(p => p.price);
                        const minPrice = Math.min(...prices);
                        const maxPrice = Math.max(...prices);
                        // Add some buffer or just use exact
                        dispatch({ type: "SET_PRICE", payload: [0, maxPrice + 100] });
                    }
                }
                    ...
                    <div className="wrapper-control-shop">
                        ...
                        {activeLayout == 1 ? (
                            <div className="tf-list-layout wrapper-shop" id="listLayout">
                                {console.log("ProductsNew rendering Listview with sorted:", sorted)}
